...
\[ 
  p(\vect \theta, \Sigma, \sigma ^2, \vect \beta_1, \ldots, \vect \beta_m, \vect y_1, \ldots, \vect y_m) = 
    p(\vect \theta) \cdot p(\vect \Sigma) \cdot 
    p(\sigma ^2) \cdot 
...
\begin{align*}
  p(\Sigma | \vect \theta, \sigma ^2, \vect \beta_1, \ldots, \vect \beta_m, \vect y_1, \ldots, \vect y_m)
    ...
    & \propto \left( 2 ^{\frac{\eta_0 p}{2}} \pi ^{\binom{p}{2} / 2} \cdot \left| S_0 \right| 
      ^{-\frac{\eta_0}{2}} \prod_{k = 1}^{p} \frac{\eta_0 + 1 - k}{2} \right) ^{-1} \cdot
      \left| \Sigma \right| ^{-\frac{\eta_0 + p + 1}{2}} \exp \left[ \frac{1}{2} \tr(S_0 \Sigma ^{-1}) \right] \cdot \\
    & \prod_j \frac{1}{\sqrt{(2 \pi) ^p \left| \Sigma \right|}} \cdot 
      \exp\left[ -\frac{1}{2} (\vect \beta_j - \vect \theta)' \Sigma ^{-1} (\vect \beta_j - \vect \theta) \right] \\
    & \propto \left| \Sigma \right| ^{-\frac{\eta_0 + p + 1}{2}} 
      \exp \left[ \frac{1}{2} \tr\left( S_0 \Sigma ^{-1} \right) \right]
      \left| \Sigma \right| ^{-\frac{m}{2}} \exp\left[ -\frac{1}{2} \sum_j 
      (\vect \beta_j - \vect \theta)' \Sigma ^{-1} (\vect \beta_j - \vect \theta) \right] \\
  ...
  \exp\left( -\frac{\nu_0 \sigma_0 ^2}{2 \sigma ^2} \right) \cdot                        \\
    ...
    & \propto (\sigma ^2) ^{- \left[ \frac{1}{2} \left( \nu_0 + \sum_j n_j \right) + 1 \right]} \cdot
      \exp\left\{ -\frac{1}{2 \sigma ^2} \left[ \nu_0 \sigma_0 ^2 +
        \textstyle \sum_j (\vect y_j - X_j \vect \beta_j)' (\vect y_j - X_j \vect \beta_j) 
      \right]\right\}
\end{align*}
...
\[ 
  ...
  \invGamma\left( 
    ...
    \frac{1}{2} \left( \nu_0 \sigma_0 ^2 + 
      \textstyle \sum_j (\vect y_j - X_j \vect \beta_j)' (\vect y_j - X_j \vect \beta_j) \right)
  \right) 
\]
...
\begin{enumerate}[label=\alph*) ]
  \item The unknown parameters in the model include population-level parameters
        ...
        \beta_1, \ldots, \vect \beta_m\} $. Draw a diagram that describes the
        relationships between these parameters, the data $ \{ y_{i, j}, x_{i, j} \}
  ...
  \item Before we do a Bayesian analysis, we will get some ad hoc estimates of
        ...
        \mintinline{R}{beta.j <- glm(y.j ~ X.j, family = binomial()) %>% coefficients}  
        Explain any problems you have with obtaining estimates for each county.
        Plot $ \expit(-\hat   \beta_{0, j}   - \hat \beta_{1, j} x) $ as a
        function of $ x $ for each county and describe what you see. Using
        maximum likelihood estimates only from those counties with 10 or more
        schools, obtain ad hoc estimates $ \hat{\vect				  \theta} $ and $  
        \hat{\Sigma} $ of $   \vect \theta $ and $ \Sigma $. Note that these
  ...
  \item Make plots of the samples of $ \vect \theta $ and $ \Sigma $ (5
        ...
        enough so that your MCMC samples are likely to be a reasonable approximation
...
\end{minted}
        ...
        \begin{figure}
          ...
          \caption{Maximum likelihood estimation model by group in scales of
            gray (the darker the line the more numerous group). Average
            coefficients for groups larger than 10 elements in red dashed line.}
...
graphics.off()
\end{minted} 
        The difference between the figures \ref{week10:groupedposterior} and
        \ref{week10:groupedglm} is evident: the estimated posterior in the
        second plot are all quite coherent and pulled to the $\beta$ population
        parameters ($ \vect   \theta $). We can note that the most relevant
        differences are in the groups with low number of observations, were the
        effect of shrinkage (that is the flow of information from the most
        numerous groups to the less numerous) is more noticeable. Moreover we
        can see that the posterior expected value of $ \vect   \theta $ is
        similar, but not equal, to the maximum likelihood statistic computed.
        \begin{figure}
          \centering
          ...
          \caption{Expected parameters given the $ \vect y_j $ by group in
            scales of gray (the darker the line the more numerous the group).
            Expected value for $   \vect \theta $ in dashed red and MLE of $ \vect
            \theta $ (same as \ref{week10:groupedglm}) in dotted brown.}
  ...
  \item Code (results in figure \ref{week10:priorvsposterior}):
        \begin{figure}
          ...
          \caption{Posterior (and some prior) marginal distributions of
            parameters: over $ \vect \theta $ values and $\Sigma$ values.}
