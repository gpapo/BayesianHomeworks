...
\begin{enumerate}[label=\alph*)]
  \item Generate $ x \sim \text{G}(\parm) $ \label{AR:step1}
  \item Generate $ u \sim \Unif(0, M g(x)) $ \label{AR:step2}
  \item Accept $ y \overset{!}{=} x $ if $ u < f(x) $ \label{AR:step3}
...
\end{enumerate}
In a generic $ u $ in the support of $ \Unif(0, M g(x)) $ the proportion of
values below $ u $ is
\[ \Pr(U < u) = \frac{u}{M g(x)} \]
...
\[ K \sim \Geom(p) \text{ with $ p $ probability of accepting at each replication} \]
We can compute the expected value of acceptance probability:
\[ p = \Pr(U \le f(x)) = \int_{S_X} g(x) \int_{0}^{f(x)} \frac{1}{M g(x)} \partial u \partial x =
