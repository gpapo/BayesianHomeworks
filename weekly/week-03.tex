...
\begin{enumerate}
 \item The inverse function of $ \logit(\cdot) $ is known to be
       ...
       \begin{align*}
        p_\psi(\psi) 
          & = \frac{1}{B(a, b)} \left(\frac{\exp(\psi)}{1 + \exp(\psi)}\right)^{a - 1}
...
\begin{enumerate}
 \item Given $ Y_1 \ldots Y_n \sim \Bernoulli(\theta) $ find the MLE of $\theta$ and
       $ \frac{J(\theta)}{n} $.
 \item Find a PDF $ p_U(\theta) $ such that $ \log p_U(\theta) = l(\theta | \vect{y}) / n + c $
...
\begin{enumerate}
 \item \begin{align*}
 	\hat{\theta}_{MLE} & = \argmin_\theta l(\theta | \vect{y})                                                     \\
 ...
 \begin{align*}
  0 & = \frac{\partial l(\theta | \vect{y})}{\partial \theta}                                   \\
 ...
 \begin{align*}
  - \frac{\partial^2 l(\theta | \vect{y})}{n \partial^2 \theta}
 ...
 \item The constrains on $ p_U(\theta) $ imply that
       ...
       \begin{align*}
        - \frac{\partial^2 \log p_U(\theta)}{\partial^2 \theta}
          & = - \frac{\partial^2 l(\theta | \vect{y}) / n + c}{\partial^2 \theta}                       \\
          & = - \frac{\partial^2 l(\theta | \vect{y})}{\partial^2 \theta} / n                           \\
          & = \frac{\theta ^ 2 - 2 \theta \hat \theta_{MLE} + \hat \theta_{MLE}}{\theta^2 (1 - \theta)^2}
       \end{align*}
 ...
 \item Same steps: simplify the log-likelihood, find the first derivative and constrain to zero.
       \begin{align*}
        \hat \theta_{MLE} & = \argmin_\theta l_\text{Poisson}(\theta | \vect{y})                               \\
