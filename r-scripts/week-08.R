...
# G prior
lmgprior <- function(y, X, g = NROW(X), nu_0 = 1,
                     s2_0 = try(summary(lm(y ~ X - 1))$sigma ^ 2, silent = TRUE),
                     niter = 5000) {
   n <- NROW(X)
   p <- NCOL(X)

   H_g   <- (g / (g + 1)) * X %*% solve(crossprod(X)) %*% t(X)
   SSR_g <- c(t(y) %*% (diag(n) - H_g) %*% y)
   s2    <- 1 / rgamma(niter, (nu_0 + n) / 2, (nu_0 * s2_0 + SSR_g) / 2)

   V_b <- g / (g + 1) * solve(crossprod(X))
...
beta_gprior <- apply(lmgpriorfit$BETA, 2, quantile, probs = c(0.025, 0.975))
...
backselect_tcrit <- function(y, X, tcrit = qnorm(0.975)) {
   ...
   repeat {
      ...
      if ( (all(abs(tstat) > tcrit) || is_empty(remain)) ) break
   }

   # Return
   list(remaining = remain, removed = setdiff(1:NCOL(X), remain))
}

lpyX <- function(y, X, g = NCOL(X), nu_0 = 1,
   ...
   H_0 <- 0
   if (p > 0) H_0 <- g / (g + 1) * X %*% solve(crossprod(X)) %*% t(X)
   SSR_g <- c(t(y) %*% (diag(n) - H_0) %*% y)
...
for (iter in 1:S) {
   for (j in sample(1:p)) {
      ...
      if (z[j] == zp[j]) lpy0 <- lpyp
   }

   beta <- z
   if (sum(z) > 0) {
      beta[z] <- lmgprior(y, X[, z, drop = FALSE], niter = 1)$BETA
   }
